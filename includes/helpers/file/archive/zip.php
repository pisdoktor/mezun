...
class mezunArchiveZip {
	...
	function _extract($archive, $destination, $options)	{
		...
		if (!$this->_data = mezunFile::read($archive)) {
		...
		for ($i=0,$n=count($this->_metadata);$i<$n;$i++) {
			if (substr($this->_metadata[$i]['name'], -1, 1) != '/' && substr($this->_metadata[$i]['name'], -1, 1) != '\\') {
				...
				$path = mezunPath::clean($destination.DS.$this->_metadata[$i]['name']);
				// Make sure the destination folder exists
				if (!mezunFolder::create(dirname($path))) {
					$this->set('error.message', 'Unable to create destination');
					return false;
				}
				if (mezunFile::write($path, $buffer) === false) {
	...
	function _extractNative($archive, $destination, $options) {
		if ($zip = zip_open($archive)) {
			if (is_resource($zip)) {
				// Make sure the destination folder exists
				if (!mezunFolder::create($destination)) {
					ErrorAlert('Unable to create destination');
					return false;
				}
				...
				while ($file = @zip_read($zip))
				{
					if (zip_entry_open($zip, $file, "r")) {
						if (substr(zip_entry_name($file), strlen(zip_entry_name($file)) - 1) != "/") {
							...
							if (mezunFile::write($destination.DS.zip_entry_name($file), $buffer) === false) {
								ErrorAlert('Unable to write entry');
								return false;
							}
							zip_entry_close($file);
						}
					} else {
						ErrorAlert('Unable to read entry');
						return false;
					}
				}
				@zip_close($zip);
			...
			ErrorAlert('Unable to open archive');
			return false;
		}
		return true;
	}
