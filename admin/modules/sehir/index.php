...
function delIl(&$cid) {
	...
	mosRedirect( 'index.php?option=admin&bolum=sehir', 'Seçili şehir(ler) silindi' );
}

function saveIl() {
	 global $dbase;
	
	$row = new Sehirler( $dbase );
	...
	mosRedirect('index.php?option=admin&bolum=sehir', 'Şehir kaydedildi');
...
function cancelIl() {
	...
	$row->bind( $_POST );
	$row->checkin();
	mosRedirect( 'index.php?option=admin&bolum=sehir');
}
